...
def logout():
    ...
    do_logout()
    flash("successfully logged out")    # we might need to add the flash-y part to login template
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        confirm_password = User.authenticate(g.user.username, form.password.data)
...
@app.route('/liking/<int:msg_id>', methods=["POST"])
def liking(msg_id):

    msg = Message.query.get(msg_id)

    if msg in g.user.messages_user_likes:
        g.user.messages_user_likes.remove(msg)    
    else:
        g.user.messages_user_likes.append(msg)
    
    db.session.commit()

    return redirect('/')

@app.route('/liking-m/<int:msg_id>', methods=["POST"])
def liking_m(msg_id):
